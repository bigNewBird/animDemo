...
public class IconCenterEditText extends EditText implements
	...
	protected void onDraw(Canvas canvas) {
		if (isLeft) { // 如果是默认样式，则直接绘制
			...
			//
			// canvas.translate((getWidth() - bodyWidth - getPaddingLeft() -
			// getPaddingRight()) / 2, 0);
			// }
			...
			// float bodyWidth = textWidth + drawableWidth + drawablePadding;
			// setPadding(getPaddingLeft(), getPaddingTop(), (int)(getWidth() -
			// bodyWidth - getPaddingLeft()), getPaddingBottom());
			// canvas.translate((getWidth() - bodyWidth - getPaddingLeft()) / 2,
			// 0);
			// }
			super.onDraw(canvas);
		}
